...
Note: `pine_get_source` can return 200KB+ on complex scripts, so avoid it unless editing.

# Writing style (all responses, docs, changelogs, comments)

Write in direct, easy-to-understand, plain English. Avoid melodramatic, flowery, or decorative language. Be informative and educational. Avoid stating the obvious or repeating points, but do not skip logical steps, ensuring explanations are thorough on the first pass. Avoid unnecessary buzzwords, but use precise domain terminology where technical accuracy requires it. Eliminate common AI mannerisms: do not overuse em-dashes (—), and avoid generic transition words (delve, tapestry, crucial, pivotal, furthermore, in conclusion).

Applying the em-dash rule in practice: use a colon to introduce, a comma or parentheses for an aside, or a full stop to split the sentence. Reserve the em-dash for genuine interruptions, and rarely.
