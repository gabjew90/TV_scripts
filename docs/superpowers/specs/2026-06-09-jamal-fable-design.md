...
**Date:** 2026-06-09 · **Status:** DRAFT for user review · **Script:** `jamal-fable.pine` (Pine v6, `overlay=true`) · **TV name:** "Jamal Fable"
...
- **BOS(up):** confirmed body close above the most recent relevant swing high. **CHoCH(down):** confirmed body close below `HL_ref`. (Mirror for downtrend.)
- **States:** `UP` (≥1 BOS up, no CHoCH since) · `DOWN` (mirror) · `CHOP` (no live regime).
- **`HL_ref` / `LH_ref`:** the most recent confirmed higher low / lower high — the trend backbone. Recomputed only when a new pivot confirms.
- **Range (CHOP):** range high = highest confirmed pivot high since the regime died; range low = lowest confirmed pivot low; both update as new pivots confirm. **No minimum-width parameter** — the ≥1.5R skip gate (§5/§6) is the width gate; a second one would be v8-style stacking.
...
- **`choch_while_open`** is logged on any open trade when a CHoCH fires (relevant to runners/trails). If backfill shows such trades bleed, CHoCH gets promoted to a forced exit **by evidence, not prose**.
...
e.g. JF|1|0.2|a3f2|B|T1|ENT|L|240|1780444800|2.627|reg=U|reg1d=C|age=37|oi_d=-1.8|q=PU.OD|fp=62|d_atr=1.4|d_pct=38|bz=1|mlh=2|rt1=2.3
```

- `max_labels_count = 500`. **Dedup key = bar time + trade type + direction** → repeated MCP harvests are idempotent.
...
- Episode simulation per symbol/trade-type is sequential: a new entry cannot open while a same-type episode is open (logged `skip_overlap`).
- Outcomes graded per §7 with distinct exit codes: `thesis_exit | stop_out | t1_hit | trail`.
...
- **v0.1 — the pipe.** Regime engine + event schema + dummy events (`SYS|PING` on each regime transition, full provenance + stub vector) emitted → MCP-harvested → parsed to JSONL → **aligned against ccxt-fetched bars**. Acceptance = the three pins: (1) timestamp-alignment check passes on every harvested event; (2) every event carries `schema_version`/script version/`settings_hash` and the parser enforces no-pool; (3) label discipline verified — one machine label per event, idempotent re-harvest via dedup key, chunked-harvest path exercised.
- **v0.2 — Trade #1 detector.** ARM/ENT/SKP/CXL events live; chart-quizzed against hand-traced examples (the Jamal-OB-style quiz loop — it caught two real rule bugs there).
