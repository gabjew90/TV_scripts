...
**Status:** superseded by v1.2.0 — the chain rule is removed, an inside candle simply ends the mono.

## Mono v1.2.0 — inside candle ends the mono; chain rule removed
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v1.2.0" (shorttitle "JMono1.2")
**User instruction (backtracking on v1.1.0's chain):** "if O is not an inside candle, it becomes the first candle of the next monolithic candle which can be either green or red. / if O is an inside candle it simply ends the previous monolithic candle."
**Rules now,** for an opposite-colour bar O with immediate predecessor P:
- O NOT inside P → O becomes the first bar of the next mono, taking O's colour. (As the base case.)
- O inside P → O is absorbed as the mono's LAST bar — it sets the mono's close — and ENDS the mono. The next bar always starts a fresh mono, whatever its colour.
**Interpretation taken, flagged:** "simply ends" is read as absorbed-and-ends (O joins the mono as its final bar), not ends-without-joining — the contrast with the not-inside case, where O explicitly starts the NEXT mono, only means anything if the inside case puts O in the PREVIOUS one.
**What changed from v1.1.0:** the chain rule (O+1 inside O also absorbed, "and so on and so forth") is deleted, and with it the `m_absorbing`/absorbing-phase state. Back to a single `m_sealed` flag: set when a bar is absorbed, forces the next bar to start a new mono, cleared on every mono start. A mono holds at most ONE absorbed bar, always the last. This is the v0.6.0 seal semantics with the v1.1.0 per-bar inside test — the two halves that survived from each attempt.
**Tests run:** compile 0/0 (buffer version VERIFIED in `.view-lines` before compiling, per the new check), on-chart version table reads v1.2.0. **NEAR.P 1D:** absorption OFF → **1,085**, the colour-only baseline, untouched. Absorption ON → **961** (v1.1.0's chain gave 952 — the chain absorbed 9 more bars across ~2,100, so the backtrack is a small tightening on this symbol).
**Not done:** no calibration case checked; symmetric-vs-low-only still open; `use_mono` in jamal-ob unaffected.
**Status:** superseded by v2.0.x — the indicator was restarted from scratch.

## Mono v2.0.0 / v2.0.1 — clean restart: inside-only grouping, overlay shading
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.0.1" (shorttitle "JMono2.0")
**User instruction:** "let's start this indicator over from scratch. get rid of everything except: inside candle logic: combine inside candles into monolithic candles, 'inside' determined by the individual candle left of it, color doesn't matter. the logic should go from right to left (most recent first), so if you have a long chain of inside candles, the more recent ones get absorbed first before later ones. for visuals, let's use overlay just like we did for Day of Week Shading."
**What the script is now — 87 lines, ONE rule, NO toggles.** A bar joins the current mono when it is inside the individual candle immediately to its left (`high <= high[1] and low >= low[1]`); colour is irrelevant; anything else starts a new mono. Everything else is gone: colour grouping, absorption phases, seals, both renderers, all grouping toggles.
**Direction note, stated to the user:** the right-to-left phrasing cannot change the result. Containment nests (C inside B inside A ⇒ C inside A), so a chain collapses to the same group whichever end is swept; the cut points are exactly the bars not inside their left neighbour. Implemented as a single forward pass — no lag, no lookahead.
**Visual:** `overlay = true`. Each mono of 2+ bars is a translucent full-height vertical band over the price candles, Day-of-Week-Shading style; single-bar monos unshaded; adjacent bands alternate between two colours. Bands are BOXES, not `bgcolor`, because bgcolor cannot be applied retroactively — a mono's first bar is only known to be in a group when its second bar arrives, and a box created then can span both. Band edges at ±half a bar (`xloc.bar_time`) so the shading covers the bars fully.
**v2.0.1 render fix, worth remembering:** v2.0.0 gave the boxes top/bottom of 1e12 / 1e-10 for "full height". They existed in the box model (`data_get_pine_boxes` returned all 335) but rendered NOTHING — TradingView culls boxes with coordinates that extreme. Replaced with a ±50x envelope around price, which covers any realistic zoom; boxes do not drive the price scale, so the bounds cannot stretch the chart.
...
**Status:** superseded by v2.2.0 — the inside test moves from the immediate predecessor to the ANCHOR.

## Mono v2.2.0 — inside is judged against the ANCHOR (left to right)
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.2.0" (shorttitle "JMono2.2")
**User instruction:** "tweak inside candle logic slightly: it doesn't apply right to left, but rather left to right, so each candle absorbs all subsequent inside candles as compared to itself."
**This one is a real semantic change,** unlike the v2.0.0 right-to-left phrasing (which was equivalent to the forward pass). The test is now against the mono's FIRST bar — the ANCHOR — not each bar's immediate predecessor: a mono is an anchor plus every consecutive bar inside the anchor's range; the first bar to poke outside starts the next mono as the new anchor. Nested chains are a strict subset of this, so absorption is strictly broader: a wide anchor now swallows a whole overlapping consolidation that the pairwise rule would cut at the first non-nested step. Since every absorbed bar is inside the anchor, the mono's high/low ARE the anchor's — absorption can never widen the mono.
**Tests run:** compile 0/0, version table v2.2.0. Offline computation on cached NEAR 1D found the discriminating case: **Aug 7-10 forms one 4-bar mono under the anchor rule (body 1.656..1.600)** where the pairwise rule stopped at Aug 7-8 (body 1.656..1.611) — Aug 9 and 10 are inside Aug 7 but Aug 9 is not inside Aug 8. On chart: `data_get_pine_boxes` contains the predicted **{1.66, 1.60}** body, and the aggregates moved the right way — **1,451 monos** (pairwise: 1,766) and **288 multi-bar boxes** (pairwise: 335), i.e. fewer, longer groups, which a strictly broader rule must produce.
**Process:** the stale-editor failure again (fourth); trigger confirmed as any editor collapse/close + reopen within a TradingView session — injection then writes to a detached model until restart. Also: crosshair pinning via synthetic clicks failed this time, so verification went through `data_get_pine_boxes` instead — reading the DRAWINGS is sturdier than reading the data window when the crosshair is uncooperative.
**Also noted:** the user pointed at the tradingview-mcp upstream repo to check for updates — see session notes; a newer upstream may fix the stale-editor bug.
**Status:** superseded by v2.3.0 — colour grouping added as a second stage.

## Mono v2.3.0 — two-stage grouping: colour merges the stage-1 monos
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.3.0" (shorttitle "JMono2.3")
**User instruction:** "now that the merge logic is in place, the same color logic depends on it before it further groups all consecutive same color candles into monolithic candles (this on top of the inside candle logic in place)."
**Grouping is now TWO STAGES, both always on:**
1. **INSIDE/ANCHOR** (v2.2.0, unchanged) — anchor plus every consecutive bar inside the anchor's range.
2. **COLOUR, on the stage-1 monos** — each stage-1 mono is RED if its close < its open (anchor's open), else GREEN (doji green, the jamal-ob convention); consecutive same-colour stage-1 monos merge into the FINAL monolithic candle. Open = first inner's open, close = last inner's close, high/low = extremes across the merged inners — so stage 2 CAN widen past a single anchor, unlike stage 1.
**The engineering problem was the live edge.** A live inner mono's close moves, so its COLOUR can flip, which attaches it to or detaches it from the final mono under construction. Solved with an F/L split: `F` holds the finalized inner monos of the current final candle (colour locked), `L` is the live inner mono. Display shows one candle for F∪L while their colours match, or F and L separately while they differ; L only becomes part of F when it FINALIZES still matching (its colour locks via `prev_l_red`, captured at the last bar before the break). Two drawing handles (boxF/boxL + wicks) are re-targeted in place on attach/detach; the frozen box is handed from L to F on commit. No lookahead anywhere; Pine's rollback covers intrabar flips.
**Tests run:** compile 0/0, version table v2.3.0, injection verified in the buffer before compiling (the fresh-editor rule held — no restart needed this time). **Offline computation on cached NEAR 1D predicted the stage-2 groups, and all four predicted bodies are in the drawn boxes at full precision:** Jul 31-Aug 1 {1.677, 1.673} · Aug 2-3 {1.735, 1.673} (two green singles merged) · **Aug 4-10 {1.736, 1.600}** — the seven-bar red final (three red singles + the red 4-bar anchor mono), the case that only exists under two-stage grouping · Aug 11-12 live {1.623, 1.600}. Screenshot confirms the candle-style render with centre wicks.
**Not done:** no calibration corpus checked against the two-stage grouping; `use_mono` in jamal-ob remains single-stage colour-only and unaffected.
**OPEN ISSUE (user-reported, unresolved at session handoff):** "why are there overlapping candles?" — the NEAR 1D screenshot shows small grey-blue boxes nested inside larger mono boxes. Suspected NOT Jamal Mono's: Jamal OB draws FVG zone boxes and HTF Candle Insights draws HTF candle boxes on the same pane, and the nested boxes match no computed Jamal Mono body. Other benign explanations: monos overlay the price candles by design; adjacent finals tile edge-to-edge; two same-colour finals can touch when an undrawn single-bar final separates them. **First diagnostic next session: hide every study except Jamal Mono and re-screenshot.** If genuinely nested mono boxes remain, suspect the attach/detach drawing-handle management in the render section.
**Status:** superseded by v2.4.0 (render moved to its own pane; grouping unchanged).

## Mono v2.4.0 — render moves to a pane below the chart; overlap report resolved (not ours)
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.4.0" (shorttitle "JMono2.4")
**User instruction:** "lets put the rendering into the pane below instead of overlaying ontop of main candles" (handoff item 2), plus the open v2.3.0 overlap investigation (item 1).
**Code changes**
- `overlay = true` → `overlay = false`. Grouping, render logic, and drawing handles untouched.
- Two invisible plots added (`plot(high/low, color.new(color.gray, 100), editable = false)`) as pane scale anchors: boxes and lines do not participate in pane autoscaling, so without a rendered plot the pane would not scale to the mono candles.
- Header comment updated to describe the pane render.
**Overlap diagnosis (open item 1) — Jamal Mono cleared.** Hid every study except Jamal Mono on NEAR 1D, then scripted a scan over all 437 drawn boxes (verbose `data_get_pine_boxes`): **zero pairs overlap by one bar or more**; adjacent finals share only the ±half-bar edge, exactly as designed. The nested grey-blue boxes in the user's screenshot belong to the other things on the pane: Jamal OB's FVG zone boxes (10 zones, several at 1.637–1.784, sitting inside the recent monos' price range), HTF Candle Insights boxes (4 zones at 1.598–1.627 around live price), plus two manual drawings that hiding studies cannot remove (the fib retracement's translucent level fill, which reads as a teal band, and an anchored volume profile, the olive blocks at the right edge). The attach/detach handle logic needed no changes.
**Process/incidents (both matter for future sessions):**
- **Stale editor, new failure mode:** after reopening a collapsed Pine panel, `pine_get_source` returned the injected v2.4.0 while the VISIBLE buffer still held v2.3.0, and the Save click saved the OLD code. So a get_source version check is NOT sufficient after any panel reopen; verify the version line visually in a screenshot. TradingView restart recovered it: the injected code survived as the restored unsaved buffer and then compiled and saved cleanly.
- **Plan limit trap:** the account is on the Essential plan (5 indicators). Removing the Jamal Mono instance to re-add it fresh (the usual way to move a study to a new pane) forfeited its slot; the re-add was blocked by the upgrade dialog. Chart undo (Ctrl+Z, one action per press) restored the removed studies. The correct move needs no remove/re-add at all: save propagates the new code to the chart instance in place, then legend → More → Move to → New pane below relocates it.
**Tests run:** compile 0/0; chart instance updated in place to v2.4.0 (same entity id LDkYeF); after the pane move `data_get_pine_boxes` still returns all 437 boxes at full precision; screenshots confirm mono candles render in the pane below with the price pane clean of them, structure matching the v2.3.0 overlay render (Aug 2-3 green merge and the seven-bar Aug 4-10 red final both present).
**Status:** superseded by v2.5.0 (single-bar finals drawn; pane gaps closed).

## Mono v2.5.0 — draw single-bar finals: the pane gaps were the undrawn singles
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.5.0" (shorttitle "JMono2.5")
**User report:** "seems like the second pane has gaps with missing candles."
**Cause:** the v2.1.0 rule "single-bar finals are not drawn (the real candle already shows them)" was correct for the overlay but wrong for the pane — there are no real candles underneath in the pane, so every single-bar final became a hole.
**Code changes**
- Removed the `f_len >= 2` guard on the standalone-F draw and the `a_len >= 2` guard on the detached-L draw; both now draw at any length. Their `box.delete` fallback branches (single-bar cleanup) went with them — no path needs deletion anymore except the attached branch dropping L's drawing.
- Added a defensive `box.set_left` to the L update path (harmless; a fresh L always gets a fresh box since finalize hands boxL to F).
- Handle-lifecycle argument for why this is safe: a detached single-bar L now always owns a box, which is exactly what the finalize hand-off (`boxF := boxL`) expects; the absorb path only fires when the display was attached on L's last bar (prev_l_red == f_red implies attached), where boxL is already deleted. Consecutive finals strictly alternate colour (stage 2 merges all same-colour neighbours), so tiling is edge-to-edge by construction.
**Tests run:** compile 0/0; buffer verified in the RENDERED view via `ui_evaluate` on `.view-lines` (per the v2.4.0 lesson, `pine_get_source` is not a valid check); chart instance updated in place. Scripted scan over the verbose box dump: **505 boxes tile bars 0..1010 with zero gaps between consecutive boxes and zero overlapping pairs** — every bar covered by exactly one mono candle. Box usage now sits at the 500 cap, so the drawn lookback is the most recent ~500 finals (~1000 bars on NEAR 1D); raise `max_boxes_count`/`max_lines_count` if deeper history is ever needed.
**Status:** superseded by v2.6.0 (stage 3 added; lone singles no longer survive).

## Mono v2.6.0 — STAGE 3: lone singles absorb backward if inside, else merge forward
**Date:** 2026-08-12 · **On-chart:** "Jamal Mono v2.6.0" (shorttitle "JMono2.6")
**User instruction:** singles like BTC 1D 2024-06-12 and 2024-08-01 "should be the open of the follow mono candle, regardless of color"; then amended: "if it's a lone inside candle it should be absorbed by the preceding mono." Re-merge question decided as option 1 (adjacent same-colour finals are NOT re-merged).
**The rule (stage 3, on stage 2's finals):** a final that completes as ONE BAR never survives, regardless of colour:
- **(a) backward:** if its range is inside the preceding final's range (`high <=` its high AND `low >=` its low), the preceding mono absorbs it — extends one bar, close becomes the single's close (its render colour can flip live since colour = direction);
- **(b) forward:** otherwise it becomes the opening bar of the following final (merged open/first-bar = the single's).
A one-bar final is always colour-sandwiched (same-colour neighbours would have merged in stage 2), so every stage-3 resolution leaves two same-colour finals adjacent; the boundary marks the swallowed single. For future stage-2 merging a final's colour stays its stage-1 members' (the swallowed single is an OHLC-only impurity). Causality: lone-ness is only known when the next differently-coloured stage-1 mono finalizes, so the live edge may display a single until then.
**Code changes**
- New stage-3 state: the committed preceding final P (`p_exists`, `p_o/c/h/l/t0/t1`) plus tracked handles `boxP`/`wickP`, so a backward absorb can re-extend P's drawing in place. P is always the newest committed final, so its box can never have been evicted by the 500 cap.
- The finalize step's else-branch (F completes) now splits three ways: `eat_back` (extend P, delete the single's drawing, L becomes new F; `f_cnt` net unchanged), `lone_f` forward (F updated in place to single+L: `f_red := prev_l_red`, extremes widened, `f_o`/`f_t0`/`f_cnt` stay the single's; L's drawing becomes the final's), else the old path (F commits and becomes P, L becomes new F).
- `half_ms` moved above the grouping block (the backward redraw needs it). Renderer unchanged.
- Handle-lifecycle safety: both stage-3 branches run only when the last render was detached (`prev_l_red != f_red`), so boxL exists and boxF is the single's own box, mirroring the pre-existing hand-off assumptions.
**Tests run:** compile 0/0; buffer verified in the rendered view (`ui_evaluate` on `.view-lines`). **On-chart (BTC 1D):** 505 boxes tile bars 0..1010 with zero gaps, zero overlaps, and ZERO one-bar boxes. **Offline computation (Binance BTCUSDT perp 1D, Apr–Sep 2024):** 16 one-bar finals pre-stage-3, 0 after; the two user examples resolve one via each clause — **2024-06-12** (lone green inside the Jun 6-11 red mono, h 72144 l 66000) absorbed BACKWARD, extending it to Jun 6-12 with close 68270.3; **2024-08-01** (low 62302 pokes below the Jul 29-31 mono's 64500.4) merged FORWARD, opening the Aug 1-7 red final at 64601.8. Both merged bodies found verbatim in the chart's drawn boxes ({71133.3, 68270.3} and {64601.8, 55102.9}).
